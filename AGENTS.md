...
**어떤 작업을 하든, 코드를 수정하거나 파일을 쓰기 전에 반드시 아래 단계를 따르세요.**

1.  **작업 유형 판단**: 현재 요청이 어떤 유형의 작업인지 결정하세요.
2.  **역할 문서 읽기**: 아래 '작업 라우팅' 테이블에서 해당 유형에 맞는 **역할 문서**를 찾아 **반드시 읽으세요**.
3.  **지침 준수**: 읽은 역할 문서의 지침에 따라 작업을 수행하세요. 역할 문서가 존재한다면, 그 문서의 지시는 이 파일의 지시보다 우선합니다.
4.  **직접 코딩 금지**: 역할 문서 없이 자신의 판단만으로 코드를 작성하거나 수정하지 마세요.
