...
- **규정**: 앱 상태는 `localStorage`의 `studio-project-widget-state-v1` 키에 저장됩니다.
- **규정**: 백업 키는 `studio-project-widget-state-v1-backup`입니다.
- **규정**: JSON 가져오기/내보내기는 `main.js`의 Electron IPC와 `state.js`의 상태 직렬화 함수를 통해 이루어집니다.
- **규정**: `getSerializableState()`가 저장하는 주요 항목은 다음과 같습니다.
    - `projects`
    - `tasks`
    - `projectLinks`
    - `archiveResources`
    ...
    - 선택된 프로젝트와 필터/보기 상태
    - 펼쳐진 프로젝트 ID
- **규정**: 직렬화 데이터에는 `version: 1`이 포함됩니다.
- **규정**: 저장 구조가 바뀌면 다음 규칙을 따릅니다.
...
- **규정**: 프로젝트는 주로 `state.js`에서 정의되고 `normalizeProjects()`로 정규화됩니다.
- **규정**: 현재 확인된 필드:
    - `id`: 숫자
    - `parentId`: 숫자 또는 `null`
    - `name`: 문자열
...
- **규정**: 현재 코드에는 별도 Folder 모델이 없습니다. `parentId`를 가진 프로젝트 계층이 폴더 역할까지 겸하는 것으로 보입니다.
- **규정**: 현재 동작:
...
- **규정**: 작업은 `state.js`에서 정의되고 `normalizeTasks()`로 정규화됩니다.
- **규정**: 현재 확인된 필드:
    - `id`: 숫자
...
- **규정**: 프로젝트 `status`는 자유 텍스트로 저장됩니다.
- **규정**: 코드상 작업 완료, 프로젝트 진행도, 마감, 하위 프로젝트에서 명확히 파생되지는 않습니다.
- **규정**: 현재는 필터와 표시 용도로 사용되는 것으로 보입니다.
- **규정**: 향후 상태 로직을 추가한다면 수동 상태인지 파생 상태인지 한 곳에서 명확히 정해야 합니다.

## 6. Progress와 Advance

- **규정**: 앱은 두 종류의 값을 구분합니다.
...
- **규정**: 저장 상태:
    ...
    - `completionWeights`
- **규정**: 중요한 위험:
    - 프로젝트 진행 값은 저장되기도 하고 계산되기도 합니다. 현재 계약은 프로젝트 저장값을 "기여 항목이 없을 때 쓰는 base/fallback 입력값"으로 취급하고, 최종 표시값은 `getProjectDisplayProgress()`와 `getProjectDisplayAdvance()`에서 가져오는 것입니다.
    - 작업 슬라이더, 하위 프로젝트, 외부 링크, 수식 링크 변경은 부모 프로젝트의 저장 `progress`/`advance`를 덮어쓰지 않고 렌더링 시 계산됩니다.

## 7. Project Links

- **규정**: `projectLinks`는 한 프로젝트가 다른 프로젝트에 주는 외부 영향입니다.
- **규정**: 필드:
    - `sourceId`
    - `targetId`
    - `metric`: `completion`, `advance`, `both`
    - `weight`: 정규화 시 5-90으로 제한
- **규정**: 이 링크는 부모/자식 계층이 아닙니다. 계층은 `parentId`가 담당합니다.

## 8. 수식 노드와 링크

- **규정**: 수식 노드는 `state.appSettings.graphFormulaNodes`에 저장됩니다.
- **규정**: 주요 필드:
    - `id`
    - `title`
    ...
    - `advance`
    - `x`, `y`
- **규정**: 수식 입력 링크는 `state.appSettings.graphFormulaInputLinks`, 수식이 프로젝트에 영향을 주는 링크는 `state.appSettings.graphFormulaLinks`에 저장됩니다.

## 9. 문서/노트/리소스 링크

- **규정**: 별도 Document 엔티티 대신 아카이브 리소스가 있습니다.
- **규정**: `archiveResources` 필드:
    - `id`
    - `name`
    ...
    - `tags`
    - `createdAt`
- **규정**: `archiveResourceLinks` 필드:
    - `resourceId`
    - `targetType`: `project` 또는 `task`
    - `targetId`
- **규정**: 그래프 전용 아카이브 노드는 `state.appSettings.graphArchiveNodes`에 있고, 그래프 링크는 `state.appSettings.graphArchiveLinks`에 있습니다.

## 10. App Settings

- **규정**: `appSettings`에는 테마, 상단 고정, 그래프 확대/축소, 노드 위치, 메모/수식/아카이브 그래프 노드, 그래프 범위, 집중 작업, 히스토리, 활동 로그, 단축키, 아카이브 보기 방식 등이 함께 들어 있습니다.
- **규정**: 설정, 레이아웃, 도메인에 가까운 그래프 노드가 한 객체에 섞여 있으므로 전체 교체나 초기화는 조심해야 합니다.
## Archive Auto-Link Data Contract

- `archiveResources`는 자료 자체를 저장한다. `path`는 열기 위한 위치 정보이며, 그래프 관계의 핵심 개념으로 쓰지 않는다.
- `archiveResourceLinks`는 자료와 프로젝트/작업 사이의 연결을 저장한다.
    - `resourceId`: `archiveResources.id`
    - `targetType`: `project` 또는 `task`
    - `targetId`: 대상 프로젝트 또는 작업 ID
- 자동 링크도 수동 링크와 같은 `archiveResourceLinks` 구조를 사용한다. 현재 모델에는 자동/수동 출처 플래그가 없으므로 자동 보강은 기존 링크를 보존하고 새 링크만 추가해야 한다.
- 자동 링크 후보 생성은 내용 중심 필터를 통과한 단서만 사용한다. 파일 타입, 드라이브명, 저장 폴더명, 스캔 출처명, 관리 태그는 후보 점수에 쓰지 않는다.
- 같은 폴더 안에서 관련 파일 후보가 3개 이상이고 해당 폴더가 `archiveResources`에 있으면, 개별 파일 링크 대신 폴더 리소스 링크를 저장한다.
- 자동 링크 정규화는 `normalizeArchiveResourceLinks()`를 거쳐 존재하지 않는 자료/프로젝트/작업 참조를 제거해야 한다.
## Archive Relation v2

- `archiveResourceLinks` keeps the existing `resourceId`, `targetType`, and `targetId` fields, and now normalizes optional relation metadata:
    - `relationStatus`: `confirmed` or `suggested`; stored links default to `confirmed`.
